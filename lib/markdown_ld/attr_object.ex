defmodule MarkdownLd.AttrObject do
  ...
  @type opts :: [strict: boolean(), max_depth: pos_integer(), max_list: pos_integer(), max_size: pos_integer()]
  ...
  def parse(str, opts \\ []) do
    ...
    if byte_size(str) > opts[:max_size], do: {:error, {:limit_exceeded, 0}}, else: do_parse(str, opts)
  end
  ...
  defp parse_key(chars) do
    {token, rest} = take_while(chars, fn c -> (c in ?a..?z) or (c in ?A..?Z) or (c in ?0..?9) or c in '._:-' or c == ?_ end)
    key = to_string(token)
    {ordered?, rest2} = if String.ends_with?(key, "[]"), do: {true, rest}, else: {false, rest}
  ...
  defp parse_value(chars, _depth, _opts) do
    {tok, rest} = take_while(chars, fn c -> not (c in ' \t\n,]}') end)
    str = to_string(tok)
    val = case String.downcase(str) do
      "true" -> true
      "false" -> false
      _ ->
        cond do
          ...
          Regex.match?(~r/^[^\s]+:[^\s]+$/u, str) -> str # CURIE or term: left for expander
          true -> str
        end
    end
    {val, skip_ws(rest)}
  ...
  # ——— helpers ———
  defp skip_ws([c | rest]) when c in '\s\t\n\r', do: skip_ws(rest)
  ...
  defp parse_string_annotation([?@ | rest]) do
    {lang, rest1} = take_while(rest, fn c -> (c in ?a..?z) or (c in ?A..?Z) or c in '-_' end)
    {{:lang, to_string(lang)}, rest1}
  end
  defp parse_string_annotation([?^, ?^ | rest]) do
    {dt, rest1} = take_while(rest, fn c -> not (c in ' \t\n,]}') end)
